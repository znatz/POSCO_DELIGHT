    ...
    <div class="pageContent">
        ...
        <div id="user_list">
            <form method="post" id="list" action="">
                ...
                echo '<table id="myTable" style="width:100%;border:0;margin:0 auto; position:relative;padding:0;border-radius:5px;" class="search_table tablesorter">';
                echo '<thead><tr>';
                ...
                foreach ($contents as $row) {
                    ...
                    echo '<td style="text-align:center;">' . $row->chrPost . '</td>';
                    echo '<td width="100px;" ' . 'title="' . $row->chrAddress . '">' . substr($row->chrAddress, 0, 9) . '...</td>';
                    echo '<td>' . $row->chrAddressNo . '</td>';
                    echo '<td style="text-align:center;">' . $row->chrTel . '</td>';
                    echo '<td style="text-align:center;">' . $row->chrFax . '</td>';
                    echo '<td style="text-align:center;">' . $row->intDisplayOrder . '</td>';
                    echo '<td style="text-align:center;"><input type="radio" onclick="javascript: submit()" name="targetID" id="targetID" value="' . $row->chrID . '"/></td>';
                    echo '<td style="padding:2px;"><input class="center_button hvr-fade delete_button" style="width:65px; height:25px; margin:0;padding:0;font-weight:normal;" type="submit" name="' . $row->chrID . '" value="削除"/></td>';
                    echo '</tr>';
                }
                echo '</tbody></table>';
